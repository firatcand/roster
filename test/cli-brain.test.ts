...
test('cli: brain init prints runtime URL exactly once; second init is a no-op (case 10)', dbOpts, async () => {
  ...
  try {
    ...
    assert.deepEqual(p1.applied, ['001_init.sql', '002_roles.sql', '003_attribution.sql', '004_documents_mount.sql']);
    ...
    assert.deepEqual(p2.skipped, ['001_init.sql', '002_roles.sql', '003_attribution.sql', '004_documents_mount.sql']);
  } finally {
    await fresh.drop();
  }
});
