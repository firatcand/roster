# Agent-Team Workspace (Dogfood Instance)

This is the dogfood instance of the agent-team pattern that `@firatcand/roster` scaffolds. It runs the same conventions as a `roster init` workspace, but it lives inside the CLI's own repo so that we eat our own cooking.

**If you're working on the CLI itself, you don't need this directory.** See `../CLAUDE.md` (repo root) for CLI development guidance. This file is loaded when Claude Code starts a session inside `.dogfood/` and below.

## Identity

You are operating inside the agent-team workspace. The workspace organizes agents by function (`gtm/`, `product/`, `design/`, `ops/`). Each agent owns its logic, tools, and per-project instances. Project-level guidelines live separately under `projects/<project>/guidelines/`.
...
- `.dogfood/.claude/` — workspace-level slash commands (`/sdr`, `/chief-of-staff`, `/dreamer`)

`.mcp.json` is discovered the same way. Agent-scoped MCPs at the agent level. Workspace MCPs at the dogfood root.
...
Two distinct kinds of intelligence live in this workspace:
...
Structural changes to the workspace (creating projects, archiving, renaming, auditing) are handled by the chief-of-staff agent at `chief-of-staff/agent.md`. When a user asks for one of these in any session, load `chief-of-staff/agent.md` and follow its operations contract.

The chief-of-staff agent operates from `.dogfood/` root only. If asked to scaffold from inside a project or agent folder, surface this and offer to switch contexts.
...
Read `conventions.md` for the full reference. If the convention isn't clear, ask before guessing — this workspace pattern is shipped to others via `roster init`, and an inconsistent convention propagates.
