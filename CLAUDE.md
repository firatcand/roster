...
- **Subcommand entry**: `src/bin/roster.ts` — hand-rolled argv parsing; subcommands `install`, `init`, `update`, `upgrade`, `doctor`, `schedule`, `review`, `skills`, `hooks`, `pending`, `migrate`, `brain`, `task`. `--help`, `--version`, exit codes (0/1/2/3).
