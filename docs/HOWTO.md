...
| `another roster migrate is writing this workspace's migration manifest` | A concurrent `roster migrate` holds the manifest lock (the error names its pid and age) — wait for it to finish. If that run crashed, delete the `.roster/migration-manifests/*.json.lock` file named in the error; locks older than 15 minutes are broken automatically. |
