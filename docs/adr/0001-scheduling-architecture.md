...
- **Failure observability (ROS-42, shipped 2026-05-18)** — dual-channel design: agent self-reports to `roster/<fn>/state.md` (orchestrator skill writes one line per fire), wrapper records process exit independently to `logs/cron/<name>.exit` (1-3 byte ASCII integer, written by `/bin/sh -c '...; printf %s "$?" > <EXIT>'` wrap around `codex exec`). `roster doctor` cross-references both signals via `auditStaleFires`: non-zero `.exit` → FAIL (flips overall ok), `detectStale` true (state.md last_run older than expected next-fire + 2h grace) → WARN. `roster pending sync` synthesizes deterministic `pending/error-<id>.md` items (`id = sha1(scheduleName + signalKey).slice(0,8)`) from those signals — idempotent re-runs from the SessionStart hook + doctor never duplicate. Optional `capture_events: true` on a codex via-cron entry adds `--json` and splits redirects (stdout → `events.jsonl`, stderr → `log`).
