...
- commit: `a0f91f950dbde47cdd3b3afe91fd8db111aee37f`
- fixture: `sha256:6e35cc3d16f008301be58d37ac9d3e9395ea46da8a06749f75952a1d3b9d85b8`
...
- effective retrieval config: rrf_k 60 (read from the Brain's own `brain_meta.config`), candidate cap 64, per-selector arm cap 8
...
| graph/unavailable-reasons-are-the-closed-pair | pass | 55 query/queries reported graph unavailable, each carrying exactly [no-cited-edge-relation, unmeasured]; availability itself is evidence, not a gate |
| families/gates-and-contracts | pass | every gated family assertion holds |
| alias-oracle/counterfactual-is-bounded | pass | every alias-oracle run stayed inside the per-arm cap with nothing truncated |
| latency/warm-max-of-five-under-remote-budget | pass | every family stayed inside the 2000 ms remote budget |

Deterministic statement cost per retrieval, on each family's representative query: baseline 10, exclusion 10, precedence 10, alias-variant 10, multi-record 10, ordering 10, paraphrase-ordering 10. Warm max-of-five `transactionMs + embedMs` per family: baseline 16 ms, exclusion 15 ms, precedence 23 ms, alias-variant 15 ms, multi-record 15 ms, ordering 23 ms, paraphrase-ordering 24 ms; report-only p95 over 30 warm runs of one representative query: 16 ms.
...
Deterministic statement cost per retrieval, on each family's representative query: baseline 10, exclusion 10, precedence 10, alias-variant 10, multi-record 10, ordering 10, paraphrase-ordering 10. Warm max-of-five `transactionMs + embedMs` per family: baseline 16 ms, exclusion 15 ms, precedence 23 ms, alias-variant 16 ms, multi-record 15 ms, ordering 26 ms, paraphrase-ordering 27 ms; report-only p95 over 30 warm runs of one representative query: 16 ms.
...
| query | miss class | alias expansions | baseline joint Recall@64 | oracle joint Recall@64 | delta | truncated | within arm cap |
| --- | --- | --- | --- | --- | --- | --- | --- |
| alias-northwind-robotics | abbreviation | 2 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-meridian-freight | abbreviation | 2 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-longview-systems | hyphenation | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-bluewater-labs | hyphenation | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-larkfield-ops | casing | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-harborpoint-media | casing | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-pinecrest-group | misspelling | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-silverbrook-holdings | misspelling | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-procurement-lead | synonym | 1 | 50.0 % | 100.0 % | 50.0 % | 0 | yes |
| alias-renewal-exposure | synonym | 1 | 50.0 % | 50.0 % | 0.0 % | 0 | yes |
...
(`delivered_set_changed = false`) while the delivered ORDER
changed: `delivered_order_changed = true`.
...
**Substrate status.** The cited engine hard-codes `graph: { status: 'unavailable', reasons: ['no-cited-edge-relation', 'unmeasured'] }` (`src/lib/brain/context-retrieval.ts:169` and `:922`). **Measured as evidence, never asserted:** 55 of 55 gold queries reported graph expansion unavailable and 0 reported it available, carrying the reason pair(s) `["no-cited-edge-relation","unmeasured"]`. The suite does **not** gate on unavailability — a future citable graph arm must be able to ship without failing this suite. The one assertion here is envelope-internal: a query that DOES report the capability unavailable must carry that closed reason pair.
