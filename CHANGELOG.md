...
- **Claude Code and Codex now receive thin, deterministic workspace activation adapters.** `roster init` and `roster update` generate one host-neutral `ROSTER.md` lifecycle plus minimal host-native pointers instead of duplicating policy across tools. The shared contract distinguishes `supported`, `advisory`, `missing`, and `drifted` capabilities, keeps the host responsible for interpreting plans and executing tools, and separates authored policy from lessons, Brain data, tool output, and diagnostics through explicit trust classes. `roster doctor --json` reports the capability catalog once and a runtime-derived activation status for each enabled host; exact supported host versions may rely on checked-in proof-scoped attestations, while unknown, unsupported, missing, or modified activation fails closed to advisory, missing, or drifted. Generated-path access failures now use the stable `FILESYSTEM_ACCESS_FAILED` diagnostic with redacted machine paths and actionable recovery. Existing workspaces adopt the generated layout through `roster update`, which preserves user-edited generated files and reports their drift. (#349)
- **Scoped workspace tool guidance resolves to one flat, safe host contract.** Tool-use definitions may now live at workspace, function, agent, or plan scope and use one canonical `<package>:<skill>` reference. Roster composes only the requested ancestry with field-level provenance, cumulative company guidance, monotone effect/approval safety, Brain intent, and a deterministic semantic hash; narrower policy cannot relax broader safety. `roster update` preflights the complete catalog before any write and atomically emits `.roster/vendor-skill-map.json`, where host-native identities remain advisory and explicitly project-materialized skills require bounded, symlink-safe per-host lock hashes. Legacy aggregate-only lock hashes fail closed until `roster skills sync` explicitly rewrites them. The unreleased v2 workspace, function, and plan schemas now require an explicit `tool_uses` list (`[]` when empty); the one-way migration remains owned by #363 rather than a compatibility reader. Claude Code or Codex still resolves and executes the vendor skill; Roster adds no provider runner, credentials, routing, health checks, fallback, or output gate. (#347)
