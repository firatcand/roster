...
/**
 * Collapse CR/LF runs to a single space for values destined for single-line
 * `#` comments in the generated install script. Without this, an embedded
 * newline would terminate the comment and let the remainder of the value
 * execute when the user runs the script. Defense-in-depth — ROS-68,
 * follow-up to ROS-64. Applies to header path values (sourceDir, destWorkspace)
 * and to the unmapped-wrapper TODO line.
 */
...
export function renderInstallScript(plan: MigrationPlan, generatedAtUtc: string): string {
  const lines: string[] = [
    ...
    `# Source: ${sanitizeForComment(plan.sourceDir)}`,
    `# Destination: ${sanitizeForComment(plan.destWorkspace)}`,
    '#',
