...
// event (append-only). FileStore is the narrow port the `brain fs` verbs and
// `brain doctor` talk to — small enough that MemoryFileStore can stand in for
// hermetic tests and doctor injection, and every backend is held to one contract.
...
async function loadS3() {
  // Lazy so non-fs commands never pay the SDK's import cost.
...
//
// fc.prefix is intentionally NOT applied here. The store treats every key as
// absolute: the prefix is baked into the key once, at derivation time, by
// `brain fs`'s deriveKey (ROS-159), and the resulting full key is persisted
// verbatim as brain.files.s3_key. Applying the prefix dynamically in the store
// would orphan every existing object the moment files.prefix changed — the same
